...
author:
  - name: Marcon Eric
abstract: >
  Modèle pour salle de classe GitHub.
...
Rédigez votre document dans ce fichier (supprimez le texte par défaut).

Ajoutez du texte et du code R dans des bouts de code.
Ajoutez les packages nécessaires dans la dernière ligne du bout de code "Options".

Tricotez le document en HTML régulièrement pour éliminer les erreurs.
Faites des *commits* fréquemment et poussez votre travail sur GitHub pour assurer sa sauvegarde.

A la fin de la rédaction, exécutez le script `GitHubPages.R` pour placer tous les fichiers produits dans le dossier `docs`, poussez et activez les pages GitHub pour votre dépôt.

Modifiez le fichier `README.md` pour qu'il décrive votre dépôt et ajoutez un lien vers le fichier <https://Utilisateur.github.io/Depot/memo.html>.


# Code

Les principales caractéristiques de Markdown sont résumées ici[^1].
